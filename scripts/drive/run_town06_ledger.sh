...
#
#   bash scripts/drive/run_town06_ledger.sh
set -uo pipefail
cd "$(dirname "$0")/.."
REPO=$PWD
...
carla_restart() {
    ...
    # (scripts/simulator/carla_restart_retry.sh). The ledger drives 24 laps with a restart before
    ...
    bash "$REPO/scripts/simulator/carla_restart_retry.sh" \
  ...
  for COND in ${TOWN06_LEDGER_CONDS:-clear fog night low_sun}; do
    ...
    for REP in $(seq 0 $((LAPS-1))); do
      for SEC in $(STUDY_MAP=Town06 python3 -c "import steering.config as C;print(' '.join(C.SECTIONS))"); do
        ...
        if ! python3 scripts/drive/closed_loop_ledger.py --student "$STU" --condition "$COND" \
...
say "LEDGER COMPLETE. Now: python3 scripts/verify/compare_town06.py"
