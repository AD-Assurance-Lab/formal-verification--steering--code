...
#
#   bash scripts/drive/finish_town06_deployment.sh
set -uo pipefail
cd "$(dirname "$0")/../.."
REPO=$PWD
...
carla_start() {
    # ONE launcher (scripts/simulator/carla_launch.sh). This function used to start CARLA itself
    ...
    say "starting CARLA on $CARLA_PORT (via scripts/simulator/carla_launch.sh)"
    if bash scripts/simulator/carla_launch.sh >>"$LOG" 2>&1; then say "CARLA ready"; return 0; fi
...
if [ "$NEED_CAPS" = "OK" ]; then
    ...
    bash scripts/capture/capture_town06_laps.sh >>"$LOG_DIR/capture.log" 2>&1 \
...
if [ -f results/arterial/captures/capture_gate.json ]; then
    ...
    python3 scripts/capture/capture_gate_drives.py >>"$LOG_DIR/capture_gate.log" 2>&1 \
    ...
    python3 scripts/capture/capture_driven_gate.py --captures results/arterial/captures \
...
python3 scripts/verify/certify_town06.py >>"$LOG_DIR/certify.log" 2>&1 \
...
if ! bash scripts/drive/run_town06_ledger.sh >>"$LOG_DIR/ledger_run.log" 2>&1; then
...
python3 scripts/verify/compare_town06.py 2>&1 | tee -a "$LOG"
