...
one it replaces.
    ...
    STUDY_MAP=Town06 python3 scripts/drive/aggregate_ledger_runs.py \
...
def main():
    ...
    for f in files:
        with open(f) as fh:
            d = json.load(fh)
        rec = dict(d["run"])
        rec["provenance"] = d.get("provenance", {})
        runs.append(rec)
...
def fold_traces():
    """One traces.csv per ledger, rebuilt from whatever per-run traces are on disk.

    closed_loop_ledger.py writes a trace per run because runs are driven one process at
    a time. Twenty-four near-identical CSVs is not a useful thing to hand a reader, so
    the committed artifact is the folded one, with a `run` column saying which lap each
    row came from. Rebuilt whole each time rather than appended, so a re-driven cell
    replaces its rows instead of doubling them.
    """
    tdir = LEDGER / "runs" / "traces"
    files = sorted(tdir.glob("*.csv")) if tdir.exists() else []
    if not files:
        return
    out = LEDGER / "traces.csv"
    with open(out, "w", newline="") as fh:
        w = None
        for q in files:
            with open(q, newline="") as g:
                rd = csv.DictReader(g)
                for row in rd:
                    if w is None:
                        w = csv.DictWriter(fh, ["run"] + rd.fieldnames)
                        w.writeheader()
                    w.writerow({"run": q.stem, **row})
    print(f"  folded {len(files)} trace(s) -> {out}")
    return 0
