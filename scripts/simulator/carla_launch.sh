...
if ! CARLA_PORT=$PORT timeout 300 python3 "$REPO/scripts/simulator/wait_carla_ready.py" --timeout 240; then
    ...
    if [ "${CARLA_WINDOWED:-0}" = "1" ]; then
        ...
        ( cd "$CARLA_ROOT" && setsid nohup ./CarlaUE4.sh -carla-rpc-port="$PORT" \
            -RenderOffScreen -quality-level="$QUALITY" $EXTRA >>"$LOG" 2>&1 < /dev/null 9>&- & )
        CARLA_PORT=$PORT timeout 300 python3 "$REPO/scripts/simulator/wait_carla_ready.py" --timeout 240 || {
...
# The GPU must be USABLE, not merely present -- see scripts/simulator/check_gpu_usable.py.
python3 "$REPO/scripts/simulator/check_gpu_usable.py" || exit 1
