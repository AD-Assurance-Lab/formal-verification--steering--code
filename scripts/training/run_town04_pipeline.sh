...
if [ ! -f "$DATA/conditions_v2/manifest.csv" ]; then
  run collect python3 scripts/training/collect_data.py --dataset conditions_v2 \
...
if [ ! -f "$CK_DIR/teacher_clear_v2_bc.pth" ]; then
  run train_clear_bc python3 scripts/training/train.py --dataset conditions_v2 --weathers clear \
...
if [ ! -f "$CK_DIR/teacher_mixed_v2_bc.pth" ]; then
  run train_mixed_bc python3 scripts/training/train.py --dataset conditions_v2 \
...
if ! ls "$CK_DIR"/teacher_mixed_v2_dagger_r*.pth >/dev/null 2>&1; then
  run dagger_mixed python3 scripts/training/dagger.py --base conditions_v2 --init teacher_mixed_v2_bc \
...
if [ ! -f "$CK_DIR/S_clear_84x28_v2.pth" ]; then
  run distill_clear python3 scripts/training/distill.py --in-w 84 --in-h 28 --out S_clear_84x28_v2 \
...
if [ ! -f "$CK_DIR/S_mixed_84x28_w3_v2.pth" ]; then
  run distill_mixed python3 scripts/training/distill.py --in-w 84 --in-h 28 --out S_mixed_84x28_w3_v2 \
...
if ! ls "$CK_DIR"/S_mixed_84x28_w3_v2_dagger_r*.pth >/dev/null 2>&1; then
  run dagger_student_mixed python3 scripts/training/dagger_student.py --student S_mixed_84x28_w3_v2 \
