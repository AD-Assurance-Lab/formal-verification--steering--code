...
for d in eastbound westbound; do
  for c in clear fog night shadows; do
    ...
    bash scripts/simulator/carla_restart.sh > "$LOGD/restart.log" 2>&1 || { echo "  restart FAILED $d/$c"; exit 1; }
    OY_OUT="$f" OY_CONDS="$c" timeout 10800 python3 scripts/capture/capture_offset_yaw.py \
