...
certificate is comparing to that tolerance.
    ...
    python scripts/verify/interpolation_fidelity.py
"""
import sys
import json
import glob
...
def check_coverage(path, sec):
    ...
    if span < MIN_COVERAGE * expect:
        print(f"\nREFUSING to measure fidelity from {Path(path).name}: it spans "
              ...
              f"Recapture with scripts/verify/capture_interp_fidelity.sh.", file=sys.stderr)
        sys.exit(2)
    return span, n
    ...
    for sec in sections:
        ...
        if not caps:
            print(f"no {args.axis} captures for section {sec} in {DIAG}. "
                  f"Run scripts/verify/capture_interp_fidelity.sh {sec}", file=sys.stderr)
            return 2
        per_sec[sec] = caps
