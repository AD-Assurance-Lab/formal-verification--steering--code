...
cap () {   # cap <out.npz> <conds> <direction> <env assignments...>
    ...
    bash scripts/simulator/carla_restart.sh > "results/arterial_logs/restart_${out%.npz}.log" 2>&1 \
        || { echo "  restart FAILED for $out"; return 1; }
    echo "  capturing $out ($conds, $dir) $*"
    env "$@" OY_CONDS="$conds" OY_OUT="$DIAG/$out" \
        python3 scripts/capture/capture_offset_yaw.py --poses 200 --direction "$dir" \
