...
python3 scripts/capture/capture_driven_gate.py --captures "$CAL" --drives results/oracle \
    2>&1 | tee "$LOG/capture_gate.log"

echo
echo "=== sustained-bias bound ==="
python3 scripts/verify/certify_sustained_bound.py "$@" 2>&1 | tee "$LOG/certify_full_route.log"
